...
public:
    GuiControllerPrivate(std::shared_ptr<TreeModel> model,
        ...
        , m_detailsDialog(new DetailsDialog(m_mainWindow))
        , m_executor(nullptr)
...
void GuiController::reportTestWidget()
{
    ...
    if (selectedBus == ADTTool::All)
    {
        ...
        QMap<QString, ADTTool::BusType> bustypes{{items[0], ADTTool::All},
                                                 {items[1], ADTTool::System},
                                                 {items[2], ADTTool::Session}};

        bool ok{};
        QString item = QInputDialog::getItem(nullptr,
                                             tr("More than one report available"),
        ...
        selectedBus = ok && !item.isEmpty() ? bustypes.value(item, ADTTool::None) : ADTTool::None;
    }
...
void GuiController::changeMode()
{
    QString toolId = getCurrentTool();
