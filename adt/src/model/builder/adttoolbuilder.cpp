                                       ...
                                       ADTTool::BusType busType)
{
    ...
    auto modesArray = (*testSection)[TESTS_CONSTANTS.MODE_KEY_NAME];
    auto mArray     = modesArray.as_array();
    if (mArray && !mArray->empty())
    {
        QStringList values;
        auto modes = modesArray.as_array();
        modes->for_each([&values](auto &elem) { values.append(elem.value_or("")); });
...
bool ADTToolBuilder::parseParamsSection(const toml::table *testSection, ADTTool *tool, const QString &varName)
{
    ...
    case ADTVarBuilderInterface::BuilderType::MODE:
        return ADTVarBuilderFactory<ADTVarBuilderInterface::BuilderType::MODE>::create()->build(testSection,
                                                                                                tool,
                                                                                                varName);
        break;
    }
