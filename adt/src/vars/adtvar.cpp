...
bool ADTVar::set(const QString &string)
{
    ...
    else if (m_type == ADTVarType::MODE)
    {
        if (m_enumStorage.get()->toList().contains(QVariant::fromValue(string)))
        {
            m_currentValue = std::make_unique<QVariant>(QVariant::fromValue(string));
...
{
    if (m_type == ADTVarType::STRING || m_type == ADTVar::ENUM_STRING || m_type == ADTVar::MODE)
    {
...
{
    if (m_type == ADTVarType::ENUM_STRING || m_type == ADTVarType::MODE)
    {
...
{
    if (m_type == ADTVarType::ENUM_INT || m_type == ADTVarType::ENUM_STRING || m_type == ADTVarType::MODE)
    {
