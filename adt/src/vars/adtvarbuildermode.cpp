#include "adtvarbuildermode.h"
#include "adtvar.h"
#include "model/builder/adtbuilderconstants.h"
#include "model/builder/adtbuilderhelper.h"

bool ADTVarBuilderMode::build(const toml::table *paramSection, ADTTool *tool, const QString &varId)
{
    ...
    if (type.isEmpty() || QString::compare(type, PARAMS_CONSTANTS.MODE_TYPE_VALUE))
    {
        qWarning() << "ERROR: Type is empty or not mode in param: " << varId << " tool with id: " << tool->id();
        return false;
    }
    ...
    vals->for_each([&stringValues](auto &elem) {
        if (elem.is_string())
        {
    ...
    if (defaultValue.is_string())
    {
        ...
        {
            bool flag = false;
            std::for_each(stringValues.begin(),
                          stringValues.end(),
                          [&flag, &stringValues, &defaultStringValue](QString &elem) {
            ...
            if (!flag)
            {
                qWarning() << "ERROR: Default value: " << defaultStringValue << "not found in values "
                           << "in param: " << varId << " tool with id: " << tool->id();
                return false;
            }
        }
    }
    ...
    {
        qWarning() << "ERROR: Type of enum is wrong (not string) in param: " << varId
                   << " tool with id: " << tool->id();
        return false;
    }
    ...
    if (defaultStringValue.isEmpty())
    {
        var.reset(new ADTVar(varId, stringValues, stringValues.at(0), displayNames, comments, true));
    }
    else
    {
        var.reset(new ADTVar(varId, stringValues, defaultStringValue, displayNames, comments, true));
    }
