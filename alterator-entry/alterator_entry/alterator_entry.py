...
def extract_packages_from_edition(
    ...
    assert sections
    internals = {}
    if "internals" in edition_data:
        internals = edition_data["internals"].value
    ...
    for internal_name in internals:
        if given_options.internal is None:
            if given_options.internal_ignore:
                break
        elif internal_name != given_options.internal:
            continue
        internal = internals[internal_name]
        internal_components = internal["components"]
        components.extend(internal_components)
