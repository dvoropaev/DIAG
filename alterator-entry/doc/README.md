...
| hidden | Помечает компонент как скрытый | boolean | нет |
...
| internals | Асоциативный массив разделов со списками компонент и пакетов для системных задач. Например, для указания перечня пакетов для третьей стадии инсталлятора | dictionary | Нет |
...
| comment               | Однострочный комментарий-описание.                                                                                                                                                                                                                                                 | string           | Нет              |
| icon                  | Отображаемая специфичная иконка. Если имя представляет собой абсолютный путь, будет использоваться переданный файл. Если имя не является абсолютным путем, для поиска значка будет использоваться алгоритм, описанный в [Спецификации темы значков](http://freedesktop.org/wiki/Standards/icon-theme-spec). | string           | Нет              |
| available_actions | Перечень доступных действий. По умолчанию считается что для сервиса доступны: `deploy`, `configure`, `backup`, `restore`, `undeploy`. Однако если это не так - можно указать явно. | array | Нет |
...
| validation_hint | Описание множества допустимых значений параметра (если оно было ограничено при помощи `pattern`). | dictionary | нет |
